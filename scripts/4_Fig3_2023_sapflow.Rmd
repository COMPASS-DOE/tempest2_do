...
```{r}
df_raw <- read_csv("../data/250502_2023_sapflow.csv") %>%  #- data not cleaned for weather
  mutate(f_avg = f_avg * 3600, 
         f_roll = f_roll * 3600)
#df_raw <- read_csv("../data/250502_2023_sapflow_cleaned.csv")
...
p1 <- ggplot(df_trim, aes(x = doy, color = plot)) + 
  ...
  labs(x = "Day of Year", y = expression("Sap flux (cm"^3*"/h)"), color = "") + 
