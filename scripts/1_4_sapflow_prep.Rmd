...
read_sapflow <- function(name){
  ...
  clean_names() %>% 
    rename("datetime_raw" = timestamp) %>% 
  filter(grepl("sapflow", research_name)) %>% 
    mutate(datetime_est = force_tz(datetime_raw, tzone = common_tz), 
           plot = plot_from_filename) %>% 
  dplyr::select(datetime_raw, datetime_est, plot, location, instrument_id, sensor_id, value, contains("f_")) %>% 
